...
MUTANTS = [
 ...
 (UI, None, "      est:!!(p.from || p.est || p.reported || p.seen || (rng && !p.upto))};",
  "      est:!!(p.from || (rng && !p.upto))};",
  'add-ons: a reported or sampled price shown without its asterisk'),
 (UI, None, "const notSold = (kind, e) => !!((tablePrice(kind, e).p || {}).not_sold);",
  "const notSold = (kind, e) => false;",
  'add-ons: an extra the airline does not sell offered at a typical price'),
 (UI, None, "    const src = p.seen ? ' own price on sample flights' : p.reported ? ' price as reported' : ' published price';",
  "    const src = ' published price';",
  'add-ons: a reported price called the airline\'s published one'),
 ("concorde-travel/addons.py", None, '"low", "high", "reported", "seen")', '"reported", "seen")',
  'add-ons: the range a source states dropped before the page'),
 (AD, "google_bag_prices", "            top = float(m.group(3) or m.group(2))\n", "            top = float(m.group(2))\n",
  'bags: a Google fee range priced at its bottom'),
 (UI, None, "if (paid.length) return {tiers:paid.map(b => b.cents), own:true, google:true, est:paid.some(b => b.range)};",
  "if (paid.length) return {tiers:paid.map(b => b.cents), own:true, google:true, est:false};",
  'bags: a Google fee range shown without its asterisk'),
 (UI, None, "  const x = ownExtras(e), own = x && (x.bags || []).filter(b => b.kind === 'checked');",
  "  const x = null, own = x && (x.bags || []).filter(b => b.kind === 'checked');",
  'bags: this flight\'s Duffel quote ignored for the fee Google lists'),
 ...
 (UI, None, "      est:!!(p.from || p.est || p.reported || p.seen || (rng && !p.upto))};", "      est:!!(p.est || p.reported || p.seen || (rng && !p.upto))};",
 ...
 (UI, None, "  return {p:row && (shortHaul(e) ? row.short : row.long),", "  return {p:row && (shortHaul(e) ? (row.short || row.long) : (row.long || row.short)),",
 ...
 (AD, "from_serpapi", 'if want and not all(c in want for c in codes):',
  ...
  'serpapi: keep an itinerary with one Delta leg among other carriers'),
    ...
    (SC, "report_card", 'pts -= _CABIN_SHORT_POINTS * share', 'pts -= 0 * share',
